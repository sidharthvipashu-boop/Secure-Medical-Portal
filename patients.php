...
/**
 * =============================================================================
 *  doctor/view_record.php  --  View one medical record (decrypt + verify)
 * =============================================================================
 *  THIS PAGE DEMONSTRATES IDEAS 1 & 2 ON READ.
 *
 *    Idea 1 (AES-256): the *_enc columns are decrypted with decrypt_data() so an
 *    authorised doctor can read the plain text. Decryption happens only here,
 *    in memory, for an authorised user -- the stored data stays encrypted.
 *
 *    Idea 2 (SHA-256): we recompute the hash of the decrypted fields and compare
 *    it to the stored record_hash.
 *        - Match    => "Integrity Verified" (data is authentic / not fake).
 *        - Mismatch => "Integrity Violation Detected" + the event is logged.
 * =============================================================================
 */
...
// Fetch the encrypted record with patient + doctor names.
$stmt = $pdo->prepare(
...
/* ---------- IDEA 1: DECRYPT the sensitive fields ---------------------------- */
...
/* ---------- IDEA 2: VERIFY the integrity hash ------------------------------- */
// Recompute the fingerprint from the decrypted text, in the SAME field order
// used when the record was created, then compare with the stored hash.
$intact = verify_integrity([$diagnosis, $treatment, $prescription, $notes], $record['record_hash']);

// If the check fails, this is a (possible) tampering event -> log it.
if (!$intact) {
    log_event($pdo, $_SESSION['user_id'], 'INTEGRITY_FAIL',
              "Integrity violation on record #$recordId (doctor view).");
}

// Reports attached to this record.
...
<div class="layout">
    ...
    <main class="content">
        ...
        <!-- Technical detail: show the actual stored ciphertext + hash (for the viva) -->
        <div class="card">
